...
export default function SeatBooker({type}) {
    ...
    const fetchData = async () =>{
        ...
        .then(data => {
            setFloor4(data.filter((seat) =>{
                return seat.floor == 4
            }))
            setFloor5(data.filter((seat) =>{
                return seat.floor == 5
            }))
    ...
    return (
        ...
        {(!floor4_res || !floor5_res && <LoadingSpinner/>) || !reservationSuccess &&
        <ResponsiveContainer>
        ...
        <div className="form-info">
            <div className="form-top">
                ...
                <div className="indicator">
                    <div style={{backgroundColor: isReserved(selectedSeat,activeLevel == 5 ? floor5_res : floor4_res) ? "#FF7C7C" : "#97FF86"}} className="indicator--icon"></div>
                    <h4>{isReserved(selectedSeat,activeLevel == 5 ? floor5_res : floor4_res) ? "Reserverad" : "Ledig"}</h4>
                </div>
                <p className="seat-information-p">{lang === "sv" ? "Mässan tar plats på våning 4 samt 5 i Täppan, Campus Norrköping. Ni bokar genom att välja en plats och våning i vår platskarta." : "The fair will take place on floor 4 and 5 in Täppan at Campus Norrköping. Book your spot by choosing floor and seat in the figure."}</p>
                </div>
            <div className="booking-form">
                <div className="flex-input">
                    <div className="div-radio">
                        ...
                        <label>Plan 5</label>
                        <input className="input-radio" type="radio" checked={activeLevel===5} onClick={() => changeFloor(5)}/>
                    </div>
                </div>
